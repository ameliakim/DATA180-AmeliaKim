...
cat("Number of objects in Vans93:", n_objects, "\n")
```
...
#  Which two Vans (give the models) have the shortest distance between them? Which two have the largest distance between them? (-2)
...
4.1
# How do you arrive at these numbers? (-1)
...
cat("Number of rows in dataframe:", num_rows, "\n")
```
...
dendrogram_euclidean <- hclust(dist(Ver_Euclidean))
plot(dendrogram_euclidean, lwd = 3, main = "Clustered Vertebrates- Euclidean Metric",
     sub = "", xlab = "Vertebrates", cex.lab = 1.25, hang = -1)
```
...
dendrogram_manhattan <- hclust(dist(Ver_Manhattan))
plot(dendrogram_manhattan, lwd = 3, main = "Clustered Vertebrates - Manhattan Metric",
     sub = "", xlab = "Vertebrates", cex.lab = 1.25, hang = -1)
```

Which method do you prefer?

I prefer the Manhattan method because it is much easier to compute the calculations by hand and it less sensitive and is not easily affected by outliers.
...
```{r}
# vertebrate <- as.data.frame(read.csv ("C:/Users/rodge/Downloads/vertebrate.csv"))
# Ver_Manhattan <- as.matrix(dist(vertebrate[, -1], method = "manhattan"))
# dendrogram <- hclust(dist(Ver_Manhattan))
# library(ggdendro)
# gg_dendrogram <- dendro_data(dendrogram, rotate = TRUE)
# 
# library(ggplot2)
# ggplot(gg_dendrogram, labels = TRUE) +
#   geom_dendrogram() +
#   theme_minimal() +
#   labs(title = "Clustered vertebrates, Manhattan Metric, Default Linkage",
#        x = "Vertebrates") +
