...
```{r}
dist_matrix <- dist(scale(Mammals_Milk))

hclust_result <- hclust(dist_matrix)
plot(hclust_result, hang = -1, cex = 1.25, sub = "", main = "Mammals Clustered by Std. Milk Composition \n Euclidean Metric, Complete Linkage", xlab = "Mammals", frame.plot = TRUE, lwd = 3)
...
set.seed(125)
library(dplyr)
MMilk_km_3 <- kmeans(scale(Mammals_Milk), centers = 3, nstart = 100)
Mammals_Milk$km_3 <- MMilk_km_3$cluster

MMilk_km_4 <- kmeans(scale(Mammals_Milk), centers = 4, nstart = 100)
...
for (k in 1:8) {set.seed(125)
  km_result <- kmeans(scale(Mammals_Milk), centers = k, nstart = 50)
  wgss[k] <- km_result$tot.withinss}

plot(1:8, wgss, type = "b", xlab = "Number of Clusters (K)", ylab = "WGSS", main = "WGSS by Number of Clusters")

#The elbow plot suggests 8 clusters. The dendrogram in Question 2 gave me the impression that there were more than 8 clusters.
```
