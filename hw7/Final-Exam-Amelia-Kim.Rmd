...
hist(loan$num_card_inq_24_month, main = "Number of credit card inquiries in last 24 months", col = "skyblue", xlab = "Number of Card Inquiries")
#Yes, the column for the number of credit card inquiries in last 24 months ('num_card_inq_24_month') is right skewed.

hist(loan$tot_amount_currently_past_due, main = "Total amount past due currently for all credit accounts", col = "salmon", xlab = "Total Amount Currently Past Due")
...
hist(loan$credit_age, main = "Age in months of first credit product obtained by the applicant", col = "lightgreen", xlab = "Credit Age")
...
ggplot(loan, aes(x = factor(def_ind_col))) +
  ...
  labs(title = "Indicator of Default", x = "Indictor of Default", y = "Frequency") +
  theme_minimal()
...
ggplot(loan, aes(x = factor(rep_edu_col))) +
  ...
  labs(title = "Education Level", subtitle = "(self-reported by applicant and not verified)", x = "Education Level", y = "Number of People") +
  theme_minimal()
...
```{r}
hist(loan$rep_income, main = "Annual Income", subtitle = "(self-reported by applicant and not
...
boxplot(tot_balance_col, main = "Total Available Balance", ylab = "Total Balance")
