...
```{r}
loan<-read.csv("loan_default_data_set.csv",header=T)
```
...
# by creating dummy variables or by assigning each category a digit (-2)
...
# Bar graphs do not communicate skewness (-1)
```
